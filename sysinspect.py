...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--no-graph",
        action="store_true",
        help=argparse.SUPPRESS,  # deprecated; meters-only is already default
    )
    ...
    show_graph = bool(force_graph or args.graph) and not args.no_graph and not plain
            ...
            if args.json:
                ...
                if plain:
                    ...
                    print(
                        short_banner(
                            color=color,
                            subtitle=f"live {title}  ·  {args.interval:g}s  ·  Ctrl+C stop",
                        )
                    )
                    print()
